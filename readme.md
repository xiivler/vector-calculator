...
* **Ground/Liquid Under Midairs**: Set to `Uniform` if the ground/liquid under the entire jump is the same height and type. Otherwise, if there is ground or liquid, set to `Varied`. A higher `Duration Search Range` is recommended for jumps over ground or liquid.
    * **Type**: Whether there is ground, lava, poison, or water underneath the jump. (Quicksand counts as regular ground.)
    * **Height**: The height of the ground, lava, poison, or water underneath the jump. You can obtain this value by standing on the ground or falling into the lava/poison, and recording Mario's Y coordinate; when falling into water, the frame in which the water begins to splash is the second frame in which Mario is below the top of the water, so you can use this information to find the top of the water. If the jump fails from landing on the ground, first make sure your initial Y coordinate is right. If the issue is still not resolved, try increasing the ground height a tiny bit.
